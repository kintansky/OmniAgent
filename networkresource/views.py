...
def allocate_public_ip_list(request):
...
def search_allocate_public_ip(request):
    context = {}
    context['public_ip'] = 1
    ip_search_form = IPsearchForm(request.GET)
    if ip_search_form.is_valid():
...
def allocate_private_ip_list(request):
...
def search_allocate_private_ip(request):
    context = {}
    context['public_ip'] = 0
    ip_search_form = IPsearchForm(request.GET)
    if ip_search_form.is_valid():
...
def allocate_public_ip(request):
    context = {}
    context['public_ip'] = 1
...
def allocate_private_ip(request):
    context = {}
    context['public_ip'] = 0
    if request.method == 'GET':
